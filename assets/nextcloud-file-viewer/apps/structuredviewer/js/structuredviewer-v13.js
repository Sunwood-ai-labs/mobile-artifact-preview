(function() {
  ...
  function readSettings() {
    ...
    if (settingsNode && settingsNode.textContent) {
      try {
        settings = { ...settings, ...JSON.parse(settingsNode.textContent) };
      } catch (error) {
        settings = { ...defaults };
      }
    }
  ...
  function applyAppearance(element, settings) {
    ...
    element.style.setProperty('--sv-accent', settings.accent || '#41d3ff');
    element.style.setProperty('--sv-highlight', settings.highlight || '#ffbe6a');
