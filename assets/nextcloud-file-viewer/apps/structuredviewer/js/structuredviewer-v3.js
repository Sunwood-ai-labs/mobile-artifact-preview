(function() {
  ...
  function renderTextPreservingHtml(value) {
    ...
    return String(value).split(htmlTagPattern).map(function(part) {
      ...
      if (htmlTagPattern.test(part)) {
        return sanitizeTrustedHtml(part);
      }
      htmlTagPattern.lastIndex = 0;
  ...
  function renderMarkdownImageBlock(line) {
    const match = /^!\[([^\]]*)\]\(([^)]+)\)\s*$/.exec(line.trim());
    if (!match) {
      return null;
    }
    return '<figure><img src="' + escapeHtml(match[2]) + '" alt="' + escapeHtml(match[1]) + '"><figcaption>' + escapeHtml(match[1]) + '</figcaption></figure>';
  }
  ...
  function renderMarkdown(markdown) {
    ...
    function flushParagraph() {
      ...
      const image = renderMarkdownImageBlock(content);
      html += image ? image + '\n' : '<p>' + renderMarkdownInline(content) + '</p>\n';
      paragraph = [];
    }
    ...
    flushList();
    flushCode();
    return '<div class="sv-panel sv-markdown">' + html + '</div>';
  }
  ...
  const StructuredViewer = {
      ...
      load: async function() {
        ...
        try {
          ...
          if (this.mime === 'application/json' || /\.json$/i.test(this.basename || this.filename)) {
            ...
            this.rendered = renderMarkdown(text);
          } else {
    ...
    render: function(createElement) {
      ...
      const title = isMarkdownFile(name, this.mime)
        ? 'Markdown Preview'
